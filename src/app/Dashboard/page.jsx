...
export default function DashboardPage() {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans overflow-x-hidden">
      ...
      {/* ✅ Mobile Overlay: ฉากหลังมืดเวลาเปิด Sidebar */}
      <div 
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:shadow-none lg:border-r lg:border-slate-800 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="h-full relative flex flex-col">
            ...
            {/* ❌ ลบปุ่มปิด (X) ตรงนี้ออกแล้ว เพื่อไม่ให้ซ้อนกับใน Sidebar.jsx */}
        ...
        <div className="p-4 md:p-8 w-full max-w-7xl mx-auto">
          ...
          <header className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center mb-6 md:mb-8">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-xl md:text-3xl font-bold text-slate-800">ภาพรวมระบบ</h1>
            ...
            <div className="flex items-center justify-end gap-2 md:gap-4 self-end md:self-auto">
              ...
              <Link href="/attendance">
                <button className="hidden md:flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:scale-105 transition-all text-sm font-bold whitespace-nowrap">
                    <Clock size={18} />
                    <span>ลงเวลาเข้างาน</span>
                </button>
                <button className="md:hidden p-2.5 bg-blue-600 text-white rounded-full shadow-md"><Clock size={20} /></button>
              </Link>
              ...
              <div className="relative">
                  ...
                  {isNotiOpen && (
                      <>
                          ...
                          <div className="absolute right-0 mt-3 w-72 md:w-96 bg-white rounded-xl shadow-xl border border-slate-100 z-20 overflow-hidden animate-in fade-in slide-in-from-top-2">
          ...
          {user?.role === 'super_admin' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
                ...
                <Link href="/hr" className="block h-full group">
                    <div className="bg-gradient-to-r from-pink-500 to-rose-500 p-4 md:p-6 rounded-2xl shadow-lg shadow-pink-200 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 relative overflow-hidden text-white h-full">
                        <div className="relative z-10 flex justify-between items-start">
                            <div>
                                <p className="text-pink-100 text-xs md:text-sm font-medium mb-1">พนักงานทั้งหมด</p>
                                <h3 className="text-2xl md:text-3xl font-bold">{loading ? "..." : dashboardData.employeeCount} <span className="text-xs md:text-sm font-normal text-pink-100">คน</span></h3>
                            </div>
                ...
                <Link href="/inventory" className="block h-full group">
                    <div className="bg-gradient-to-r from-orange-400 to-amber-500 p-4 md:p-6 rounded-2xl shadow-lg shadow-orange-200 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 relative overflow-hidden text-white h-full">
                        <div className="relative z-10 flex justify-between items-start">
                            <div>
                                <p className="text-orange-100 text-xs md:text-sm font-medium mb-1">สินค้าใกล้หมด</p>
                                <h3 className="text-2xl md:text-3xl font-bold">{loading ? "..." : dashboardData.stockAlert} <span className="text-xs md:text-sm font-normal text-orange-100">รายการ</span></h3>
                            </div>
                            <div className="p-2 md:p-3 bg-white/20 rounded-xl backdrop-blur-sm">
                ...
                <Link href="/sales" className="block h-full group">
                    <div className="bg-gradient-to-r from-purple-500 to-indigo-600 p-4 md:p-6 rounded-2xl shadow-lg shadow-purple-200 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 relative overflow-hidden text-white h-full">
                        <div className="relative z-10 flex justify-between items-start">
                            <div>
                                <p className="text-purple-100 text-xs md:text-sm font-medium mb-1">ยอดขายวันนี้</p>
                                <h3 className="text-2xl md:text-3xl font-bold">฿{loading ? "..." : Number(dashboardData.todaysSales).toLocaleString()}</h3>
                            </div>
                            <div className="p-2 md:p-3 bg-white/20 rounded-xl backdrop-blur-sm">
                ...
                <Link href="/hr/approve-leave" className="block h-full group">
                    <div className="bg-gradient-to-r from-cyan-400 to-teal-500 p-4 md:p-6 rounded-2xl shadow-lg shadow-cyan-200 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 relative overflow-hidden text-white h-full">
                        <div className="relative z-10 flex justify-between items-start">
                            <div>
                                <p className="text-cyan-100 text-xs md:text-sm font-medium mb-1">คำขอลา (รออนุมัติ)</p>
                                <h3 className="text-2xl md:text-3xl font-bold">{loading ? "..." : leaveStats.pendingLeaves}</h3>
                            </div>
                            <div className="p-2 md:p-3 bg-white/20 rounded-xl backdrop-blur-sm">
                ...
                <>
                  ...
                  <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-2">
                          ...
                          <button className="text-slate-400 hover:text-slate-600 bg-slate-50 p-2 rounded-lg self-end sm:self-auto"><MoreHorizontal size={20}/></button>
                      </div>
                      
                      {/* Responsive Chart Container */}
                      <div className="relative h-48 md:h-64 w-full bg-indigo-50/30 rounded-2xl overflow-hidden border border-indigo-100">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-3">
                             ...
                             <div><p className="text-xs text-slate-500">รายรับรวม</p><p className="text-lg md:text-xl font-bold text-slate-800">฿425,000</p></div>
                          </div>
                          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-3">
                             <div className="p-2 bg-orange-100 text-orange-600 rounded-lg"><PieChart size={20}/></div>
                             <div><p className="text-xs text-slate-500">กำไรสุทธิ</p><p className="text-lg md:text-xl font-bold text-slate-800">฿120,500</p></div>
                          </div>
                      </div>
                  </div>

                  {/* --- Admin Timeline --- */}
                  <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 flex flex-col h-full min-h-[400px] md:min-h-[500px]">
                                     ...
                                     <div key={act.id} className="ml-6 relative group">
                                         ...
                                         <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 hover:bg-white hover:shadow-md transition cursor-default">
                                              <div className="flex justify-between items-start mb-1">
                                                 <h4 className="text-xs font-bold text-slate-800 line-clamp-1">{act.title}</h4>
                                                 <span className={`text-[9px] font-bold px-1.5 rounded uppercase shrink-0 ${act.status==='success'?'bg-green-100 text-green-600':'bg-slate-200'}`}>{act.status}</span>
                                              </div>
