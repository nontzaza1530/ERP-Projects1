...
"[project]/src/app/api/production/projects/[id]/route.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    ...
    try {
        ...
        const [costsResult, logsResult, plansResult, membersResult] = await Promise.all([
            connection.query("SELECT * FROM project_costs WHERE project_id = ? ORDER BY recorded_date DESC", [
                id
            ]),
            connection.query("SELECT * FROM production_logs WHERE project_id = ? ORDER BY log_date DESC", [
                id
            ]),
            connection.query("SELECT * FROM production_plans WHERE project_id = ? ORDER BY planned_date ASC", [
                id
            ]),
...
async function PUT(request, { params }) {
    ...
    try {
        ...
        if (status) {
            await connection.query('UPDATE projects SET status = ? WHERE id = ?', [
                status,
                id
            ]);
        } else {
            await connection.query('UPDATE projects SET project_name = ?, customer_name = ?, due_date = ? WHERE id = ?', [
                project_name,
                customer_name,
                due_date,
                id
...
async function DELETE(request, { params }) {
    ...
    try {
        ...
        try {
            // 1. ลบตารางลูกก่อน
            await connection.query('DELETE FROM project_costs WHERE project_id = ?', [
                id
            ]);
            await connection.query('DELETE FROM production_logs WHERE project_id = ?', [
                id
            ]);
            await connection.query('DELETE FROM production_plans WHERE project_id = ?', [
                id
            ]);
            await connection.query('DELETE FROM project_members WHERE project_id = ?', [
                id
            ]);
            // 2. ลบโปรเจกต์หลัก
            await connection.query('DELETE FROM projects WHERE id = ?', [
                id
            ]);
            // ✅ ยืนยันการลบทั้งหมด
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b2512af3._.js.map