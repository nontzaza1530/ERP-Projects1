...
async function POST(request) {
    ...
    try {
        ...
        // ✅ 2. บันทึกหัวบิล PO
        const [resHeader] = await connection.query(`INSERT INTO purchase_orders (po_number, supplier_id, order_date, expected_date, total_amount, created_by, status, remarks, shipping_address) 
         VALUES (?, ?, ?, ?, ?, ?, 'Pending', ?, ?)`, [
            poNumber,
            supplier_id,
            order_date,
        ...
        if (ref_pr_id) {
            await connection.query(`UPDATE purchase_requests SET status = 'PO_Created' WHERE id = ?`, [
                ref_pr_id
            ]);
        }
